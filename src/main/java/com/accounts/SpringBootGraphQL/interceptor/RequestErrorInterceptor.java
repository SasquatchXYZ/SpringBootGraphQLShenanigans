...
public class RequestErrorInterceptor implements WebGraphQlInterceptor {
    ...
    private WebGraphQlResponse processResponse(WebGraphQlResponse response) {
        if (response.isValid()) {
            ...
            return response.transform(builder -> builder.errors(modifiedErrors));
        }
    }
    ...
    private GraphQLError createValidationError(ResponseError error) {
        ...
        if (error.getMessage().contains("is not a valid 'CountryCode'")) {
            ...
            extensionMap.put("Supported Country Codes", "International country codes are shot alphanumeric combinations that uniquely identify countries or geographical areas around the world.");
